...
class AzureBlobStorageClient:
    ...
    def upload_file_with_metadata(self, bytes_data, file_name, content_type='application/pdf', metadata=None):
        blob_client = self.blob_service_client.get_blob_client(container=self.container_name, blob=file_name)

        # Set content settings and metadata
        content_settings = ContentSettings(content_type=content_type)
        if metadata is not None:
            content_settings.metadata = metadata

        # Upload the created file
        blob_client.upload_blob(bytes_data, overwrite=True, content_settings=content_settings)
